...
def validate_timestamp(timestamp):
    """
    Validate and convert a timestamp to datetime object.
    
    Args:
        timestamp: A timestamp which could be a string or datetime object
    ...
    if isinstance(timestamp, str):
        try:
            ...
            # If not ISO format, try common date formats
            formats = [
                "%Y-%m-%d %H:%M:%S",
                "%Y-%m-%d %H:%M:%S.%f",
                "%Y-%m-%dT%H:%M:%S",
            ...
            for step, logs in step_to_logs.items():
                for log in logs:
                    if hasattr(log, 'timestamp') and log.timestamp is not None:
                        ...
                        ts = validate_timestamp(log.timestamp)
                        if ts is not None:
            ...
            for step_num, step in step_dict.items():
                ...
                for log in step_logs:
                    if hasattr(log, 'is_error') and log.is_error and hasattr(log, 'timestamp') and log.timestamp:
                        ...
                        cluster_points[cluster_id].append(ts)
...
def generate_timeline_image(step_to_logs, step_dict, output_dir, test_id):
    ...
    Returns:
        Path to the generated image or placeholder if generation fails
    """
    # Acquire lock to prevent concurrent generation of the same visualization
    with _visualization_locks["timeline"]:
        ...
        temp_debug.append(f"Step count: {len(step_dict) if step_dict else 0}")
        
        try:
            # Create figure
            fig = plt.figure(figsize=(12, 8))
            
            # Validate input data - note we don't return early anymore, but we'll adapt the visualization
            if not step_to_logs:
                logging.warning("No step_to_logs data provided for timeline visualization")
                temp_debug.append("WARNING: No step_to_logs data provided")
            
            if not step_dict:
                logging.warning("No step_dict data provided for timeline visualization")
                temp_debug.append("WARNING: No step_dict data provided")
                # Create an empty step_dict to avoid errors
                step_dict = {}
            ...
            if len(all_timestamps) < 2 and step_to_logs:
            ...
            if not all_timestamps:
                ...
                steps = sorted(step_dict.keys()) if step_dict else []
                if not steps and step_to_logs:
                    steps = sorted(step_to_logs.keys())
                ...
                if steps:
                    ...
                    # No steps at all - create a placeholder visualization
                    plt.text(0.5, 0.5, "No step data available for timeline visualization", 
            ...
            # Draw step boundaries
            for step_num, step in step_dict.items():
                step_logs = step_to_logs.get(step_num, []) if step_to_logs else []
                ...
                for log in step_logs:
                    if hasattr(log, 'is_error') and log.is_error and hasattr(log, 'timestamp') and log.timestamp:
                        ...
                        if severity == 'high':
                            error_points_high.append(ts)
                            has_high = True
                        elif severity == 'low':
                            error_points_low.append(ts)
                            has_low = True
                        else:
                            error_points_med.append(ts)
                            has_medium = True
            ...
            # If placeholder generation failed or is disabled, provide a simple fallback
            if not placeholder_path:
                logging.info("Placeholder generation failed or disabled, creating simple fallback")
                try:
                    # Create simple fallback image
                    fig = plt.figure(figsize=(8, 6))
                    plt.text(0.5, 0.5, f"Timeline visualization error: {str(e)}", 
...
class VisualizationGenerator(ReportGenerator):
    ...
    def generate(self, data: ReportData) -> Dict[str, str]:
        ...
        try:
            ...
            if hasattr(data, "step_to_logs") and data.step_to_logs and hasattr(data, "step_dict") and data.step_dict:
                ...
                timeline_path = self._generate_with_timeout(
                    visualization_paths,
                    "timeline",
                ...
                # If timeline generation failed or is not in paths dict, create a placeholder
                if "timeline" not in visualization_paths:
                    placeholder_path = generate_placeholder(
                        base_output_dir, 
                        self.config.test_id, 
                        "Timeline visualization failed to generate"
                    )
                    if placeholder_path:
                        visualization_paths["timeline"] = placeholder_path
    ...
    def _generate_with_timeout(self, paths_dict, key, feature_flag, default_value, timeout_seconds, func_name, *args, **kwargs):
        ...
        if path and os.path.exists(path):
            ...
            # Always ensure we have a valid path for timeline visualization (Module 2 requirement)
            if key == "timeline":
                # Generate a placeholder as fallback
                placeholder_path = generate_placeholder(
                    args[0], args[1], f"Timeline visualization fallback"
                )
                if placeholder_path:
                    paths_dict[key] = placeholder_path
                    return placeholder_path
