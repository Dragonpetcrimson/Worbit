...
def generate_step_report(
    ...
    if not timeline_step_dict:
        logging.error("Failed to build timeline step dictionary. Timeline visualization will fail.")
    ...
    if enable_images and timeline_step_dict:  # Only try if we have valid step data
        try:
        ...
        if not timeline_step_dict:
            logging.error("Cannot generate timeline due to missing step metadata")
    ...
    if timeline_image_path:
        ...
        image_relative_path = f"supporting_images/{image_filename}"
        logging.info(f"Using relative path for timeline image: {image_relative_path}")
    else:
        image_relative_path = None
        logging.warning("No timeline image path available for HTML embedding")
        ...
        if image_relative_path:
            ...
            html += f"""
                <div id="timeline-section" class="timeline-container">
                    ...
                    <img src="{image_relative_path}" alt="Test Execution Timeline" class="timeline-image" />
                    <p><em>{timeline_description}</em></p>
                </div>
            """
        else:
            # No valid image path - include detailed error message
            error_reason = ""
            if not enable_images:
                error_reason = "Timeline visualization is disabled in configuration."
            elif not timeline_step_dict:
                error_reason = "No valid step metadata could be extracted from logs."
            else:
                error_reason = "An error occurred during timeline generation. Check logs for details."
            ...
            # Add debug information in HTML comments
            html += f"""
                <!-- 
                Timeline Debug Info:
                - Enable Images: {enable_images}
                - Steps with Metadata: {len(timeline_step_dict) if timeline_step_dict else 0}
                - Timeline Type: {timeline_type}
                - Image Path: {timeline_image_path if timeline_image_path else 'None'}
                -->
            """
